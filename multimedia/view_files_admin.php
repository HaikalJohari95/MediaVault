<?php
// ============================================================
//  MediaVault Admin - Global Detailed Metadata & Repository Control
// ============================================================
...
$currentUserRole = strtolower(trim((string) ($_SESSION['access_role'] ?? $_SESSION['role'] ?? 'Admin')));

// Guard: Kill script execution if a non-administrative user tries to access this catalog
if ($currentUserRole !== 'admin') {
    die("Access Denied: You do not possess adequate security authorization clears to inspect the global repository.");
}
...
// ADMIN RESOLUTION: We remove individual user bounds ('m.user_id = ?') to fetch globally
$where = [];
$params = [];
$types = '';
...
// 1. Total records count globally
...
// 2. Summary cards count globally
$typeCounts = ['Document' => 0, 'Audio' => 0, 'Video' => 0];
$typeStmt = mysqli_prepare($conn, "SELECT file_type, COUNT(*) AS total FROM multimedia_files GROUP BY file_type");
...
// 3. Main structural query joining user account details for admin auditing view
$sql = 'SELECT m.file_id, m.file_name, m.file_type, m.size_kb, m.stored_path, m.upload_timestamp, u.username, u.department 
...
<head>
    ...
    <title>Global Repositories Control (Admin) - MediaVault</title>
    <style>
        :root {
            --bg: #eef2f7;
            --surface: #ffffff;
        ...
        * { box-sizing: border-box; }
        ...
        .container { max-width: 1200px; margin: 0 auto; padding: 34px 18px 42px; }
        .page-header {
            background: #111827; /* Darker header variant indicating Admin view space */
        ...
        .brand-mark { color: #fca5a5; font-size: 12px; font-weight: 800; letter-spacing: 0.9px; margin: 0 0 7px; text-transform: uppercase; }
        h1, h2 { margin: 0; }
        h1 { color: #fff; font-size: 31px; line-height: 1.15; }
        h2 { color: #152238; font-size: 21px; margin-bottom: 8px; }
        .page-header .muted { color: #e5e7eb; margin-bottom: 0; }
        .panel {
        ...
        .badge { var(--primary-soft); border: 1px solid #c9d9ff; border-radius: 999px; color: #1e40af; display: inline-block; font-size: 12px; font-weight: 800; padding: 5px 10px; }
        ...
        /* Direct Inline Audio / Video Previewer Styles */
        .inline-previewer {
            max-width: 260px;
            min-width: 200px;
        }
        .inline-previewer video {
            width: 100%;
            max-height: 120px;
            border-radius: 6px;
            background: #000;
        }
        .inline-previewer audio {
            width: 100%;
            height: 32px;
        }
    ...
    <main class="container">
        <header class="page-header">
            <div class="header-row">
                <div>
                    <p class="brand-mark">MediaVault Administration Dashboard Space</p>
                    <h1>Global Repositories Control</h1>
                    <p class="muted">System-wide operational oversight: audit file instances, streaming status, owners, and structural metadata records.</p>
                </div>
                <a class="btn secondary" href="../index_admin.php">Back to Admin Panel</a>
            </div>
        </header>

        <section class="panel">
            <h2>System Asset Catalog Matrix</h2>
            <p class="muted">Monitor and stream all content segments registered cross-account inside database tables.</p>

            <div class="type-summary">
                <a class="type-card" href="view_files_admin.php?type=Document" style="text-decoration:none;">
                    <span>Global Documents</span>
                    <strong><?php echo $typeCounts['Document']; ?></strong>
                </a>
                <a class="type-card" href="view_files_admin.php?type=Audio" style="text-decoration:none;">
                    <span>Global Audio Tracks</span>
                    <strong><?php echo $typeCounts['Audio']; ?></strong>
                </a>
                <a class="type-card" href="view_files_admin.php?type=Video" style="text-decoration:none;">
                    <span>Global Video Reels</span>
            ...
            <form class="filters" method="get">
                ...
                <select name="sort">
                    ...
                    <option value="sort" <?php echo $sort === 'size' ? 'selected' : ''; ?>>Sort by size</option>
                ...
                <a class="btn secondary" href="view_files_admin.php">Reset</a>
            </form>
            ...
            <div class="table-wrap">
                <table>
                    <thead>
                        <tr>
                            <th>ID</th>
                            <th>File Name / Owner</th>
                            <th>Type</th>
                            <th>Size</th>
                            <th>Upload Date</th>
                            <th style="min-width: 240px;">Live Player Stream Preview</th>
                            <th>Action</th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php if ($result && mysqli_num_rows($result) > 0): ?>
                            <?php while ($row = mysqli_fetch_assoc($result)): 
                                $cleanPath = htmlspecialchars($row['stored_path'] ?? '');
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <div style="font-size: 12px; margin-top:4px; color: var(--muted);">
                                            Uploader: <span class="badge admin-user"><?php echo htmlspecialchars($row['username'] ?? 'System/Unknown'); ?></span>
                                        </div>
                                    </td>
                                    <td><span class="badge"><?php echo htmlspecialchars($fileType); ?></span></td>
                                    ...
                                    <td>
                                        <?php if (!empty($cleanPath)): ?>
                                            <div class="inline-previewer">
                                                <?php if ($fileType === 'Video'): ?>
                                                    <video controls preload="none">
                                                        <source src="<?php echo $cleanPath; ?>" type="video/mp4">
                                                        <source src="<?php echo $cleanPath; ?>" type="video/webm">
                                                        Your browser does not support video playbacks.
                                                    </video>
                                                <?php elseif ($fileType === 'Audio'): ?>
                                                    <audio controls preload="none">
                                                        <source src="<?php echo $cleanPath; ?>" type="audio/mpeg">
                                                        <source src="<?php echo $cleanPath; ?>" type="audio/wav">
                                                        Your browser does not support audio elements.
                                                    </audio>
                                                <?php else: ?>
                                                    <a class="btn success" style="min-height:auto; padding:6px 12px; font-size:12px;" href="<?php echo $cleanPath; ?>" download>💾 Download Document Asset</a>
                                                <?php endif; ?>
                                            </div>
                                        <?php else: ?>
                                            <span class="badge missing">Metadata Only (No Path)</span>
                                        <?php endif; ?>
                                    </td>
                                    
                                    <td>
                                        <a class="btn secondary" style="min-height:auto; padding:8px 12px; font-size:13px;" href="file_details_admin.php?id=<?php echo (int) $row['file_id']; ?>">Inspect Details</a>
                                    </td>
                                </tr>
                            <?php endwhile; ?>
                        <?php else: ?>
                            <tr><td colspan="7" class="muted" style="text-align:center; padding: 30px;">No matching global multimedia assets found in system records.</td></tr>
                        <?php endif; ?>
                    </tbody>
                </table>
            </div>
            ...
            <?php if ($totalRows > $perPage): ?>
                <div class="pagination">
                    <span class="muted">
                        ...
                        of <?php echo $totalRows; ?> tracked server items
                    </span>
                    <div class="page-links">
                        <?php if ($page > 1): ?>
                            <?php $queryParams['page'] = $page - 1; ?>
                            <a class="btn secondary" href="view_files_admin.php?<?php echo htmlspecialchars(http_build_query($queryParams)); ?>">Previous</a>
                        <?php endif; ?>

                        <?php for ($i = 1; $i <= $totalPages; $i++): ?>
                            <?php $queryParams['page'] = $i; ?>
                            <a class="btn <?php echo $i === $page ? 'active' : 'secondary'; ?>" href="view_files_admin.php?<?php echo htmlspecialchars(http_build_query($queryParams)); ?>"><?php echo $i; ?></a>
                        <?php endfor; ?>

                        <?php if ($page < $totalPages): ?>
                            <?php $queryParams['page'] = $page + 1; ?>
                            <a class="btn secondary" href="view_files_admin.php?<?php echo htmlspecialchars(http_build_query($queryParams)); ?>">Next</a>
                        <?php endif; ?>
                    </div>
                </div>
            <?php endif; ?>
