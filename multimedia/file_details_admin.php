<?php
// ============================================================
//  MediaVault Admin - Detailed Metadata & Media Preview Control
// ============================================================
...
// Extract and clean requested file identifier
...
// Fetch individual file detailed metrics joining user account details
...
// Guard: Throw validation error if entry is not present in database records
if (!$fileData) {
    die("Error: The requested file profile does not exist or has been dropped from system tables.");
}
    ...
    <main class="container">
        <header class="page-header">
            <div class="header-row">
                <div>
                    <p class="brand-mark">File Inspector Audit Engine</p>
                    <h1><?php echo $fileName; ?></h1>
                    <p class="muted">Asset Identity Reference Pointer: #<?php echo (int)$fileData['file_id']; ?></p>
                </div>
                <a class="btn secondary" href="view_files_admin.php">Return to Matrix</a>
            </div>
        </header>
        ...
        <section class="panel">
                ...
                <div class="player-stage">
                    <?php if ($fileType === 'Video'): ?>
                        <video controls preload="auto">
                            ...
                            Your server environment fallback statement: Browser engine lacks capability for native HTML5 video stream decoding.
                        </video>
                    <?php elseif ($fileType === 'Audio'): ?>
                        <audio controls preload="auto">
                            <source src="<?php echo $cleanPath; ?>" type="<?php echo $mimeType ?: 'audio/mpeg'; ?>">
                            Your server environment fallback statement: Browser engine lacks capability for native HTML5 audio stream decoding.
                        </audio>
                    <?php else: ?>
            ...
            <div class="meta-grid">
                ...
                <div class="meta-item">
                    ...
                    <div class="meta-value"><span class="badge admin-user"><?php echo htmlspecialchars($fileData['username'] ?? 'System / Orphaned Profile Reference'); ?></span></div>
                </div>
                <div class="meta-item">
            ...
            <div class="meta-item" style="margin-top: 16px; word-break: break-all;">
                <div class="meta-label">Internal Server Absolute Filepath Directory Pointer Location</div>
                <div class="meta-value" style="font-family: monospace; font-size: 13px; color: #0f172a; margin-top: 4px;"><?php echo $cleanPath ? $cleanPath : 'No server relative path registered inside system rows.'; ?></div>
            </div>
        </section>
    </main>
</body>
